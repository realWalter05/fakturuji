...
def fill_out_items(sheet, items, start_row, description):
    ...
    for item in items:
        ...
        if item.delivery_name:
            sheet["A" + str(item_number)] = item.delivery_name

        if item.count:
            sheet["E" + str(item_number)] = return_number(item.count)

        if item.price:
            sheet["F" + str(item_number)] = return_number(item.price)
...
def get_dph_rates(sheet, items, start_row, items_count, description):
    ...
    dph_rates = {}
    for item in items:
        if not item.dph.replace(',', "").replace('.', "").isnumeric():
            continue
        dph = return_number(item.dph)
...
def write_qr_platba_code(sheet, start_row, account_number, bank_code, items, var_cislo, prenesena_dph):
    ...
    if not prenesena_dph:
        for item in items:
            value = (return_number(item.price) + (return_number(item.price) * return_number(item.dph) * 0.01)) * return_number(item.count)
            ammount += value
    else:
        for item in items:
            value = return_number(item.price) * return_number(item.count)
            ammount += value
